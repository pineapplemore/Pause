...
enum ReportPDFService {
    ...
    static func generate(
        ...
        let tags = appState.tagDistribution(from: range.start, to: range.end)
        let periodName = period.displayName(isChinese: isChinese)
        let title = isChinese ? "放屁报告" : "Puff Report"
        ...
        return renderPDF(
            ...
            isChinese: isChinese,
            tagDisplay: { tagId in
                if let t = PuffType(rawValue: tagId) { return t.displayName(isChinese: isChinese) }
    ...
    private static func renderPDF(
        ...
        return renderer.pdfData { context in
            ...
            if !tagDistribution.isEmpty {
                let tagTitle = isChinese ? "标签分布" : "Tag distribution"
                (tagTitle as NSString).draw(at: CGPoint(x: margin, y: y), withAttributes: attrsHead)
                y += lineHeight
                for (tagId, count) in tagDistribution.prefix(12) {
                    let name = tagDisplay(tagId)
