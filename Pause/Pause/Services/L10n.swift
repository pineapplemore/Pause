...
enum L10n {
    ...
    static func allBehaviors(_ isChinese: Bool) -> String { isChinese ? "全部" : "All" }
    static func periodComparison(_ isChinese: Bool) -> String { isChinese ? "周期对比" : "Comparison" }
    static func behaviorTrendTitle(_ isChinese: Bool) -> String { isChinese ? "所有行为日折线图对比" : "All behaviors daily comparison" }
    static func selectDateForHourly(_ isChinese: Bool) -> String { isChinese ? "选择日期" : "Select date" }
    static func hourlyDistribution(_ isChinese: Bool) -> String { isChinese ? "每日时段分布" : "Hourly distribution" }
    static func hourlyHint(_ isChinese: Bool) -> String { isChinese ? "可看出一天中哪个时段记录最多" : "Peak hours in a day" }
    ...
    /// 按记录时的名称分组；若曾改名，历史与新记录会分开显示
    static func behaviorDistributionHint(_ isChinese: Bool) -> String { isChinese ? "按记录时的行为名称统计；若曾修改过某行为名称，历史记录会与新记录分开显示。" : "Counts by name at record time; if you renamed a behavior, old and new records show separately." }
    static func showTagDistribution(_ isChinese: Bool) -> String { isChinese ? "显示行为分布" : "Show behavior distribution" }
    ...
    static func summaryPhrase(_ isChinese: Bool, _ count: Int) -> String {
        if isChinese {
            if count == 0 { return "本周期暂无记录。" }
            return "本周期共 \(count) 次记录。"
        } else {
            if count == 0 { return "No records this period yet." }
            return "\(count) records this period."
        }
    }
    static func noRecordsToday(_ isChinese: Bool) -> String { isChinese ? "今天还没开张" : "No records yet today" }
    ...
    static func behaviorsSectionTitle(_ isChinese: Bool) -> String { isChinese ? "行为" : "Behaviors" }
    static func addBehavior(_ isChinese: Bool) -> String { isChinese ? "添加行为" : "Add behavior" }
    static func showInitialOnWidget(_ isChinese: Bool) -> String { isChinese ? "在小组件按键右下角显示首字" : "Show initial on widget button" }
    ...
    static func widgetFavoriteTagsSubtitle(_ isChinese: Bool) -> String { isChinese ? "在小组件上显示的行为（最多 4 个），点击即可记录" : "Behaviors on widget (up to 4); tap to record" }
    ...
    static func widgetTagDeviceHint(_ isChinese: Bool) -> String { isChinese ? "小组件标签点击记录仅适用于 iOS 17 等支持交互小组件的机型。" : "Widget quick tags work on iOS 17 and other devices with interactive widgets." }
    ...
    static func milestoneMessage(_ isChinese: Bool, _ count: Int) -> String {
        if isChinese {
            ...
            switch count {
            case 10: return "You've logged 10. The journey continues!"
            case 50: return "50! You're halfway there."
            case 100: return "100! Your report will remember."
            case 500: return "500 reached. Full power!"
            case 1000: return "1000. You're a legend."
